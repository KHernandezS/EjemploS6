		 ...
		 <div class="container-fluid"  style="margin: 50px 0;">
            <div class="row">
                <div class="col-xs-12 col-sm-4 col-md-3">
                    <img src="../assets/images/aula.png" alt="user" class="img-responsive center-box" style="max-width: 110px;">
                </div>
                <div class="col-xs-12 col-sm-8 col-md-8 text-justify lead">
                    Bienvenido a la sección para registrar nuevos salones, debes de llenar todos los campos del siguiente formulario para poder registrarlo.
                </div>
            </div>
        </div>
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-xs-12 lead">
				<ol class="breadcrumb">
				  <li class="active">Listado de salones</li>
				  <li><a href="seccion.php?op=nuevo">Nuevo salon</a></li>
				</ol>
			</div>
		</div>
	</div>
...
<div class="container-fluid">
		  ...
		  <input id="FiltrarContenido" type="text" class="form-control" placeholder="Buscar salón" aria-label="Tema" aria-describedby="basic-addon1">
		</div>

		<button onclick="window.location.href='reporteseccion.php'" tooltips-general="PDF" type="button" class="btn btn-secundary" style="margin-top:-60px; margin-left:1000px;"><i class="zmdi zmdi-print zmdi-hc-fw"></i>&nbsp;&nbsp; PDF</button>
		<button onclick="window.location.href='excelseccion.php'" tooltips-general="EXCEL" type="button" class="btn btn-secundary" style="margin-top:-105px; margin-left:1070px;"><i class="zmdi zmdi-print zmdi-hc-fw"></i>&nbsp;&nbsp; EXCEL</button>
             ...
             <table class="table table-striped">
                        ...
                        <tbody class="BusquedaRapida">
                          ...
                          foreach ($dato as $key => $value){
                              foreach ($value as $va) { ?>
                           <tr>
                              <td><?php echo $va['codsec'];?></td>

                              <td><?php echo $va ['nomsec'];?></td>
							  <td><?php echo $va ['capacidad'];?></td>
							  <td><?php echo $va ['ubicacion'];?></td>
                              <!--<td><?php echo $va ['nomclase'];?></td>
							  <td><?php echo $va ['nomprof'];?></td>-->
							  
                              <td>
									<?php    if($va['estado']==1)  { ?> 
									<form  method="get" action="javascript:activo('<?php echo $va['codsec']; ?>')">
										<button type="submit" class="btn btn-success btn-xs">Activo</button>
									</form>
									<?php  }   else {?> 

									<form  method="get" action="javascript:inactivo('<?php echo $va['codsec']; ?>')"> 
								...
								<td>
								<a href="#ver_<?php echo $va["codsec"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></a>
								
								<a href="#delete_<?php echo $va["codsec"]; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
								<?php include('../vista/seccion/BorrarEditarModal.php'); ?>
							
								<a href="#file_<?php echo $va["codsec"]; ?>" title="Editar" data-backdrop="false" class="btn btn-success btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-picture"></span></a>
								<?php include('../vista/seccion/EditarModal.php'); ?>
...
function activo(coddsec)
{
	var id=coddsec;
	$.ajax({
        type:"GET",
		url:"../assets/ajax/editar_estado_activo_usuario1.php?id="+id,
    }).done(function(data){
...
function inactivo(coddsec)
{
	var id=coddsec;
	$.ajax({
		type:"GET",
		url:"../assets/ajax/editar_estado_inactivo_usuario1.php?id="+id,
    }).done(function(data){
        window.location.href ='../folder/seccion.php';
    })
}
