	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-xs-12 lead">
				<ol class="breadcrumb">
				  <li class="active">Listado de instructor</li>&nbsp;&nbsp;
				 <!-- <li><a href="docente.php?op=nuevo">Nuevo docente</a></li> -->
				 <a href="../vista/docente/nuevo.php">Nuevo instructor</a>
...
?>
	...
	<div class="container-fluid">
            <h2 class="text-center all-tittles">Listado de instructores</h2>
		  ...
		  <input id="FiltrarContenido" type="text" class="form-control" placeholder="Buscar instructor" aria-label="Docente" aria-describedby="basic-addon1"> 
		</div>
		<button onclick="window.location.href='reportedocente.php'" tooltips-general="PDF" type="button" class="btn btn-secundary" style="margin-top:-60px; margin-left:1000px;"><i class="zmdi zmdi-print zmdi-hc-fw"></i>&nbsp;&nbsp; PDF</button>
		<button onclick="window.location.href='excelasdocente.php'" tooltips-general="EXCEL" type="button" class="btn btn-secundary" style="margin-top:-105px; margin-left:1070px;"><i class="zmdi zmdi-print zmdi-hc-fw"></i>&nbsp;&nbsp; EXCEL</button>
             ...
             <table class="table table-striped">
                        ...
                        <tbody class="BusquedaRapida">
                          ...
                          foreach ($dato as $key => $value){
                              foreach ($value as $va) { ?>
                           <tr>
                              <td><?php echo $va['coddoce'];?></td>

							<td><?php  echo "<img src='../assets/images/imagenes/".$va['foto']."'width='90'"; ?></td>
                              <td><?php echo $va ['nomprof'];?> <br> <i class="zmdi zmdi-face"></i>&nbsp;
							  <?php echo $va ['nomusu'];?></td>
							  <td><i class="zmdi zmdi-email"></i>&nbsp;<?php echo $va ['email'];?><br>
							  <b><i class="zmdi zmdi-smartphone-iphone"></i>&nbsp;<?php echo $va ['telef'];?></td></b>
						...
						<td>
             <?php    if($va['estado']==1)  { ?> 
              <form  method="get" action="javascript:activo('<?php echo $va['coddoce']; ?>')">
                <button type="submit" class="btn btn-success btn-xs">Activo</button>
              </form>
            <?php  }   else {?> 

              <form  method="get" action="javascript:inactivo('<?php echo $va['coddoce']; ?>')"> 
            ...
            <?php  } ?>                         
								...
								<a href="#ver_<?php echo $va["coddoce"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></a>
								
								
								
								<a href="#delete_<?php echo $va["coddoce"]; ?>" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
							<?php include('../vista/docente/BorrarEditarModal.php'); ?>
							
							<a href="#file_<?php echo $va["coddoce"]; ?>" title="Editar" data-backdrop="false" class="btn btn-success btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-picture"></span></a>
								<?php include('../vista/docente/EditarModal.php'); ?>
								
								
	

						
				<!--<a href="#password_<?php echo $va["coddoce"]; ?>" title="Password" data-backdrop="false" class="btn btn-success btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-lock"></span></a>-->
				<?php include('../vista/docente/Modalpassw.php'); ?>
	...
	function activo(coddoce)
{
	...
	$.ajax({
        type:"GET",
		url:"../assets/ajax/editar_estado_activo_usuario.php?id="+id,
    }).done(function(data){
...
function inactivo(coddoce)
{
	...
	$.ajax({
		type:"GET",
		url:"../assets/ajax/editar_estado_inactivo_usuario.php?id="+id,
    }).done(function(data){
        window.location.href ='../folder/docente.php';
    })
}
